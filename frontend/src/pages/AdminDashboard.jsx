...
function AdminDashboard() {
  ...
  const chartCardClass =
    "rounded-[1.7rem] border border-gray-100 bg-white p-5 shadow-[0_14px_35px_rgba(15,23,42,0.07)] dark:border-[#1f3a40] dark:bg-[#0f2428] dark:shadow-[0_14px_35px_rgba(0,0,0,0.28)]";
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-2xl text-sm font-medium text-gray-500 dark:text-slate-300">
            Real-time appointment insights, status tracking, top doctors, and
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {cards.map((card) => (
            <div
              key={card.label}
              className="rounded-[1.5rem] border border-gray-100 bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.07)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_50px_rgba(15,23,42,0.10)] dark:border-[#1f3a40] dark:bg-[#0f2428] dark:shadow-[0_14px_35px_rgba(0,0,0,0.28)] dark:hover:border-[#46daea]/25 dark:hover:shadow-[0_22px_50px_rgba(0,0,0,0.38)]"
            >
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-gray-400 dark:text-slate-500">
                    {card.label}
                  </p>

                  <p className={`mt-2 text-4xl font-black ${card.color}`}>
                    {card.value}
                  </p>
                </div>

                <div
                  className={`flex h-14 w-14 items-center justify-center rounded-2xl ${card.bg} ${card.color}`}
                >
                  {card.icon}
                </div>
              </div>
            ...
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-gray-500 dark:text-slate-400">
                  Monthly booking activity from recent appointment data.
                </p>
              </div>
          ...
          <div className={chartCardClass}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={4}
                  >
                    {analytics.appointmentsByStatus.map((entry) => (
                      <Cell
                        key={entry.status}
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              {analytics.appointmentsByStatus.map((item) => (
                <div
                  key={item.status}
                  className="flex items-center gap-2 rounded-2xl bg-gray-50 px-3 py-2 text-xs font-bold text-gray-600 dark:bg-[#071416] dark:text-slate-300"
                >
                  <span
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
          <div className={chartCardClass}>
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-gray-500 dark:text-slate-400">
                  Most booked doctors on the platform.
                </p>
              </div>
                ...
                analytics.topDoctors.map((doctor, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#e8fbfd] text-sm font-black text-[#008e9b] dark:bg-[#46daea]/15 dark:text-[#46daea]">
                        #{index + 1}
                      </div>
                    ...
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#008e9b] dark:bg-[#0f2428] dark:text-[#46daea]">
                      {doctor.appointments}
                    </span>
                  </div>
                ))
          ...
          <div className={`${chartCardClass} xl:col-span-2`}>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-[#1f3a40]">
